...
class _SingleSelectionBoxState extends State<SingleSelectionBox>
  ...
  void initState() {
    ...
    if (context.findAncestorWidgetOfExactType<GroupSharedData>()!.selected ==
        widget.id) {
      ...
      return GestureDetector(
        child: AnimatedBuilder(
          ...
          builder: (context, child) {
            var _shrinkAnimation =
                Tween<double>(begin: 1, end: 0.9).animate(CurvedAnimation(
              ...
              Positioned.fill(
                ...
                child: AnimatedBuilder(
                  ...
                  builder: (context, child) {
                    ...
                    num topRightSquredDis =
                        pow(constraints.maxWidth - pressedX, 2) +
                            pow(pressedY, 2);
                    num botLeftSquredDis = pow(pressedX, 2) +
                        pow(constraints.maxHeight - pressedY, 2);
                    num botRightSquredDis =
                        pow(constraints.maxWidth - pressedX, 2) +
                            pow(constraints.maxHeight - pressedY, 2);
                    num radius = sqrt(max(
                        max(topLeftSquredDis, topRightSquredDis),
                        max(botLeftSquredDis, botRightSquredDis)));
                    var _opacityAnimation = Tween<double>(begin: 1, end: 0)
                        .animate(_opacController);
                    return OverflowBox(
                      maxHeight: radius * 2,
                      maxWidth: radius * 2,
                      child: SizedBox(
                ...
                builder: (context, child) {
                  ...
                  return ClipRRect(
                    child: Transform.translate(
                      ...
                      offset: Offset(pressedX - constraints.maxWidth / 2,
                          pressedY - constraints.maxHeight / 2),
                    ),
